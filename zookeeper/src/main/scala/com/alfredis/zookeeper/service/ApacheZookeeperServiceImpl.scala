...
case class ApacheZookeeperServiceImpl(keeper: ZooKeeper, events: Hub[WatcherEvent]) extends ApacheZookeeperService {
  ...
  override def getDataAsString(path: String): IO[DomainError, ZookeeperNode] = ZIO.async { callback =>
  ...
  override def getChildrenWithData(path: String, eventType: WatcherEventType): IO[DomainError, Map[String, ZookeeperNode]] = {
    getChildren(path, true, eventType).flatMap { children =>
      ZIO
        .collectAll {
          children.map(child => getDataAsString(s"$path/$child").map(node => child -> node))
        }
        .map(_.toMap)
    }
  }
